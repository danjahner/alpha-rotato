...
echo $CAS_TO_ROTATE | jq -r .[] | while read object; do
  ...
  ./credhub set --type certificate --name $object --certificate "$(./credhub g -n $object --versions 2 --output-json | jq -r .versions[1].value.certificate & ./credhub g -n $object --versions 2 --output-json | jq -r .versions[0].value.certificate)" --private "$(./credhub g -n $object --output-json | jq -r .value.private_key)"
done
